...
export default injectIntl(function AccountCrossWithdrawals({ accountId, intl: { messages } }) {
  const [{ tableData }, setState] = useRedux("accountCrossWithdrawals", { tableData: {} });

  const tableService = useMemo(
    () => new TableService(api.fetchBtcWithdrawals$, tableData, { accountId: accountId }),
    []
  );
